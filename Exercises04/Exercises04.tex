...
%
\begin{align*}
	(\y_{nr} | \f_{nr}, \sigma^2) &\sim \pN \left( \f_{nr}, \sigma^2 \mathcal{I} \right) \\
	(\f_{nr} | \g_{n}, \bm{\uptheta}_f) &\sim \pN \left( \g_{n}, \K_f(\tee_{nr}, \tee_{nr}) \right) \\
	(\g_{n} | \h_i,\bm{\uptheta}_g) &\sim \pN \left( \h_i, \K_g(\tee_{nr}, \tee_{nr}) \right)\\
	(\h_i | \tee_{nr}, \bm{\uptheta}_h) &\sim \pN \left( \mathbf{0}, \K_h(\tee_{nr}, \tee_{nr}) \right),
\end{align*}
%
%
where $\bm{\uptheta}_f$, $\bm{\uptheta}_g$, and $\bm{\uptheta}_h$ are vectors containing the respective parameters for the covariance functions $k_f$, $k_g$, and $k_h$. Let $\bm{\uptheta} = (\bm{\uptheta}_f, \bm{\uptheta}_g, \bm{\uptheta}_h, \sigma^2)^T.$ It is straightforward to find the marginal likelihood of the data $\y_{nr}$,
%
%
\begin{align*}
...
%
%
Now we consider the full data vector for all genes in cluster $i$, $\YY_i = \left\{ \y_n \right\}_{n\in c_i}$ where each $\y_n$ is a concatenation of the replicates in gene $n$, $\y_n = \left\{ \y_{nr} \right\}_{r=1}^{N_n}$. We can also write $\YY_i = \left\{ \left\{ \y_{nr} \right\}_{r=1}^{N_n} \right\}_{n\in c_i}.$ Note that $\tee_n = \left\{ \tee_{nr} \right\}_{r=1}^{N_n} =: \tee$ is the same for each $n$ as explained above, and has length $D$. Finally, define $\mathbf{T}_i = \left\{ \tee_n \right\}_{n \in c_i}$. This will have a marginal full likelihood,
%
%
\begin{align*}
...
%
\begin{align*}
	\Sigma_i[n, n'] &= \text{cov}(\y_{n}, \y_{n'}) = \begin{cases}
...
%
\begin{align*}
	\Sigma_n[r, r'] &= \text{cov}(\y_{nr}, \y_{nr'}) = \begin{cases}
...
Now the concern is choice of hyperparameters, $\bm{\uptheta}$. We have two options here.
%
%
\begin{enumerate}[(1)]
	\item \textbf{Fully Bayesian:} Place a hyperprior on $\bm{\uptheta}$ (perhaps a joint independent half-Cauchy priors for each component) and then find the posterior using Bayes' Rule.
	\item \textbf{Marginal Likelihood Maximization:} Maximize $\log p(\YY_i | \bm{\uptheta}, \mathbf{T}_i)$ w.r.t. $\bm{\uptheta}$.
\end{enumerate}
%
%
For the sake of simplicity in implementation, I chose the latter approach. The downside is that the likelihood is not convex, so we are not guaranteed to find a global minimum. One approach to this problem is using gradient based methods, using the partial derivatives,
%
%
\begin{align*}
	\frac{\partial}{\partial \theta_j} \log p(\YY_i | \bm{\uptheta}, \mathbf{T}_i)  &= \frac{\partial}{\partial \theta_j} \left[ -\frac{1}{2}\YY_i^T \Sigma_i^{-1} \YY_i - \frac{1}{2} \log \left| \Sigma_i \right | - \frac{N_iD}{2} \log 2\pi \right] \\
	&= -\frac{1}{2} \YY_i \Sigma_i^{-1} \frac{\partial \Sigma_i}{\partial \theta_j} \Sigma_i^{-1} \YY_i - \frac{1}{2} \text{tr} \left( \Sigma_i^{-1} \frac{\partial \Sigma_i}{\partial \theta_j} \right) \\
	&= -\frac{1}{2} \text{tr} \left( \left[\bm{\upeta} \bm{\upeta}^T - \Sigma_i^{-1} \right]\frac{\partial \Sigma_i}{\partial \theta_j} \right), \text{ where } \bm{\upeta} = \Sigma_i^{-1} \YY_i
\end{align*}
%
%
for each component $\theta_j$ in $\bm{\uptheta}$ (notice that $\frac{\partial \Sigma_i}{\partial \theta_j}$ is a matrix derivative). In my \textsf{R} script, however, I used the \texttt{optim} command to do this, which by default uses the Nelder-Mead method of optimization. For this application, we use the squared-exponential covariance function with zero ``nugget'' parameter, i.e.,
%
%
\begin{align*}
	k_h(t, t') &= \alpha_h \cdot \exp\left[ -\left( \frac{t - t'}{\gamma_h} \right)^2 \right] \\
	k_g(t, t') &= \alpha_g \cdot \exp\left[ -\left( \frac{t - t'}{\gamma_g} \right)^2 \right] \\
	k_f(t, t') &= \alpha_f \cdot \exp\left[ -\left( \frac{t - t'}{\gamma_f} \right)^2 \right],
\end{align*}
%
%
where each $\alpha_{\bullet}$ is called the \emph{amplitude} parameter, and each $\gamma_{\bullet}$ is called the \emph{relative length} parameter. \\
...
Parameter & MLE (group 1) & MLE (group 2) & MLE (group 3) \\ \hline
 $\gamma_f$  & $1.96 \times 10^{+2}$ & $1.53 \times 10^{-2}$ & $2.26 \times 10^{-1}$ \\ 
  $\alpha_f$ & $1.44 \times 10^{-4}$ & $2.24 \times 10^{-2}$ & $1.01 \times 10^{-2}$ \\ 
  $\gamma_g$ & $3.48 \times 10^{+0}$ & $2.10 \times 10^{+0}$ & $1.38 \times 10^{+0}$ \\ 
  $\alpha_g$ & $3.68 \times 10^{-2}$ & $1.26 \times 10^{+0}$ & $1.42 \times 10^{+0}$ \\ 
  $\gamma_h$ & $7.85 \times 10^{+1}$ & $2.96 \times 10^{+1}$ & $1.13 \times 10^{+1}$ \\ 
  $\alpha_h$ & $2.20 \times 10^{+4}$ & $1.48 \times 10^{+2}$ & $1.41 \times 10^{+2}$ \\ 
  $\sigma^2$ & $1.99 \times 10^{-3}$ & $1.91 \times 10^{-1}$ & $3.72 \times 10^{-2}$ \\ 
\end{tabular}
	\caption{Results of Nelder-Mead method for finding MLEs of hyperparameters}
\end{table}
