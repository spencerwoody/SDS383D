...
\begin{enumerate}[(A)]
  ...
  &= \begin{bmatrix}
	...
	%
	%
	The figure below shows the fitted model with 95\% confidence bands and overlaid scatterplot of the data. The optimal bandwidth $h = 5.4168$ was chosen with leave-one-out cross validation. The fit is pretty good, with $R^2 = 0.88$, but there are four observations which fall outside the confidence band.
	%
	%
	%
	\begin{figure}[htp!]
...
\begin{enumerate}[(A)]
	...
	%
	%
	We have the likelihood $y | \theta \sim \pN_n (R\theta, \Sigma)$, with $R$ a $n \times p$ matrix, and the prior distribution $\theta \sim \pN_p(m, V)$. Let $\Omega = \Sigma^{-1}$ and $W = V^{-1}$. We can expand out the respective PDFs of these distributions up to a proportionality constant,
	%
	%
	%
	\begin{align*}
	...
	\begin{align*}
		...
		&\propto \exp\left[ -\frac{1}{2} \left( \theta^T \left[W + R^T \Omega R \right] \theta - 2m^TW\theta + y^T \Omega y - 2y^T\Omega R\theta \right) \right],
	\end{align*}
	%
	%
	%
which may be factorized so that the joint distribution is
	%
	%
	%
	\begin{align*}
		\begin{bmatrix}
			y \\
			\theta
		\end{bmatrix} &\sim \pN_{n+p} \left( \begin{bmatrix}
			Rm \\
			m
		\end{bmatrix}, \begin{bmatrix}
			\Omega & -\Omega R \\
			-R^T \Omega & W + R^T\Omega R
		\end{bmatrix}^{-1} \right).
	\end{align*}
	%
	%
...
\begin{enumerate}[(A)]
	...
	%
	\begin{align}
		E(\f^\star | \y) &= C_\star (C + \sigma^2I)^{-1} \y \\
		\text{cov}(\f^\star | \y) &= C_{\star \star} - C_\star (C + \sigma^2I)^{-1} C_\star^T.
	\end{align}
	%
	%
	%
	Note that the expectation is a linear smoother on $\y$.
	%
	%
	%
	\item % C
	...
	%
	%
	The first figure below shows the choice of tuning parameters, and the next figure shows the fitted GP for the utilities data.
	%
	%
	%
	\begin{figure}[htp!]
		\centering
			\includegraphics[scale=0.5]{img/tune1.pdf}
		\caption{Choice of hyperparameters}
	\end{figure}
	%
	\begin{figure}[htp!]
		\centering
			\includegraphics[scale=0.5]{img/Minn_GP.pdf}
		\caption{Fitted Gaussian Process for Minnesota utilities dataset}
	\end{figure}
	%
	%
	%
	...
	\begin{figure}[htp!]
		\centering
			\includegraphics[scale=0.45]{img/tplot.pdf}
			\includegraphics[scale=0.45]{img/pplot.pdf}
		\caption{Scatterplot of Temperature (left) and Pressure data}
	\end{figure}
	%
	\begin{figure}[htp!]
		\centering
			\includegraphics[scale=0.5]{img/GPtemp.pdf}
		\caption{Fitted GP of Temperature Data}
	\end{figure}
	%
	%
	%
