...
\begin{enumerate}[(A)]
	...
	\begin{figure}[htp!]
		\centering
			% \includegraphics[scale=0.5]{opth3.pdf}
	...
	\begin{figure}[htp!]
		\centering
			\includegraphics[scale=0.5]{2x2.pdf}
...
\begin{enumerate}[(A)]
	...
	%
	%
	With $H$ a smoothing matrix (or ``hat matrix''), let $r = y - Hy$ be the vector of residuals. If the random vector $x$ with mean vector $\mu$ and covariance matrix $\Sigma$, then $E(x^TQx) = \text{tr}(Q\Sigma) + \mu^T Q \mu$. Then, 
	%
	%
	%
	\begin{align*}
